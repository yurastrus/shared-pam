...
def api_next_verification_segment(lang_code):
    ...
    try:
        ...
        # Log the full traceback for better diagnostics.
        current_app.logger.error(traceback.format_exc())
...
def api_submit_verification(lang_code):
    """API endpoint to save verification results."""
    conn = None
    try:
        data = request.json
        if not data:
...
def api_verification_stats(lang_code):
    ...
    try:
        ...
        # --- Query 1: Verified segment statistics ---
        ...
        # --- Query 2: Count remaining segments for verification ---
        ...
        # --- Query 3: Per-species statistics ---
...
def _confine_to_pam_base(path):
    ...
    Returns realpath if it is within the base; otherwise abort(403). Guards against
    a tampered file_path in the DB (insider / future SQLi / compromised admin)
    that would otherwise serve arbitrary server files. If PAM_UPLOAD_PATH is not
    configured, logs an error and passes through to avoid regressing existing behaviour.
    """
...
def serve_verification_audio(lang_code, segment_id):
    """Serve audio files for segment verification."""
    conn = None
    try:
        conn = get_pam_db_connection()
        ...
        # #26 (SEC-013): confirm the path is inside PAM_UPLOAD_PATH;
        # a tampered DB file_path could otherwise serve arbitrary server files.
...
def api_detailed_evaluation_results(lang_code):
    """API endpoint to fetch detailed evaluation results with pagination."""
    from .utils import get_pam_db_connection
...
def admin_recalculate_metrics(lang_code):
    """Admin route to recalculate all evaluation metrics."""
    ...
    try:
        ...
        if species_choice and species_choice != 'all':
            try:
                ...
                pass  # Garbage input falls back to None (i.e., 'all').
        ...
        if result['success']:
            ...
            # 1. Top-level summary.
            if calc_count == 0:
                # Nothing calculated — not an error, but not success either.
                if mode == 'single':
            ...
            # 2. Skipped-species details — specific reasons.
            ...
            # Structured error — severity level depends on reason.
...
def admin_cleanup_verifications(lang_code):
    """Admin route to delete files for completed verifications."""
...
def api_consensus_status(lang_code):
    """API endpoint to fetch verification consensus statistics."""
    from .utils import get_pam_db_connection
...
def serve_spectrogram_image(lang_code, segment_id):
    """Serve spectrogram images for segments."""
    conn = None
    try:
        conn = get_pam_db_connection()
        ...
        # #26 (SEC-013): confirm the path is inside PAM_UPLOAD_PATH
        # (guards both spectrogram generation and serving the image from that directory).
        _confine_to_pam_base(audio_path)

        base_path, _ = os.path.splitext(audio_path)
        spectrogram_path = f"{base_path}.png"

        # If the image is missing, generate it on the fly.
        ...
        # Check again that the file exists after generation.
        if not os.path.exists(spectrogram_path):
...
def process_spectrograms_background(app, segments, force_regenerate):
    # App context needed for logger and config access.
    with app.app_context():
        ...
        for segment_id, audio_path in segments:
            ...
            # It handles existence check and overwrite logic internally.
...
def admin_build_spectrograms(lang_code):
    """Admin route to trigger asynchronous spectrogram generation."""
    conn = None
    try:
        ...
        conn = get_pam_db_connection()
        # Fetch all segment file paths.
        segments = conn.execute(text(
            "SELECT id, file_path FROM segments WHERE status != 'archived'"
        )).fetchall()
        
        # Convert RowProxy to plain tuples so they are safe to pass to the thread
        # (connection close has no effect on the copied data).
        segments_data = [(row[0], row[1]) for row in segments]
        
        # --- START IN BACKGROUND ---
        # Use threading to return a response immediately.
        # Pass current_app._get_current_object() so the thread has access to the real app.
...
def api_get_evaluation_thresholds(lang_code):
    """
    API endpoint to fetch a dict of p0.95 threshold values for species.
    Only includes calculations with total_samples > 200.
    Returns: { "scientific_name": threshold_value, ... }
    """
    conn = None
    try:
        conn = get_pam_db_connection()
...
def manage_pam_locations(lang_code):
    ...
    try:
        ...
        # 1. Fetch ALL institutions from the main DB. 
        if is_admin:
        ...
        # 2. Fetch locations (with both name variants) from the PAM DB.
        loc_query = text("""
        ...
        # 3. Fetch location–biotope links.
        ...
        # 7. Reference data for the service log.
...
def get_pam_location_details(lang_code, location_id):
    """API endpoint to fetch details for a specific PAM location."""
    g.lang_code = lang_code
    conn = None
    try:
        conn = get_pam_db_connection()
...
def pam_location_coverage(lang_code, location_id):
    """Location recording-coverage calendar by day (Idea 10).

    Day coverage = number of distinct hours (0..24) with ≥1 recording. Honest estimate
    without assumptions about recording duration (recordings does not store a duration field).
    """
    g.lang_code = lang_code
    conn = None
    try:
...
def api_create_pam_location(lang_code):
    """API endpoint to manually create a new PAM location."""
    g.lang_code = lang_code
    conn = None
    try:
        data = request.json
...
def update_pam_location(lang_code, location_id):
    ...
    try:
        ...
        with conn.begin(): 
            ...
            # Update institution links in the PAM DB (overwrite IDs).
            if is_admin:
...
def admin_convert_to_flac(lang_code):
    """Admin route to convert WAV segments to FLAC."""
...
def api_top_verifiers(lang_code):
    ...
    try:
        ...
        # Apply the same filters as for segments.
        species_id = request.args.get('species_id', type=int)
        status = request.args.get('status', 'all')
        ...
        # Two-step process:
        # 1. Fetched user IDs and statistics from PAM_DB.
        # 2. Now fetch usernames from the main database.
        user_ids = [row[0] for row in db_results]
        
        # Query the main database for usernames via the User model.
        users = User.query.filter(User.id.in_(user_ids)).all()
        user_map = {u.id: u.username for u in users}
...
def pam_trends(lang_code):
    """Render the multi-year species trend visualisation page."""
    g.lang_code = lang_code
    conn = None
    try:
        conn = get_pam_db_connection()
        
        # Fetch species that have calculated trends.
        species_query = text("""
            SELECT DISTINCT s.species_id, s.scientific_name, s.common_name_uk, s.common_name_en
            FROM species s
...
def api_pam_yearly_trends(lang_code):
    """API endpoint to fetch data for the multi-year dynamics chart."""
    g.lang_code = lang_code
    conn = None
    try:
        species_id = request.args.get('species_id', type=int)
        ...
        # Convert result rows to a list of dicts for JSON serialisation.
...
def pam_species_dashboard(lang_code):
    """Render the PAM species detailed analysis page (seasonal + annual dynamics)."""
    g.lang_code = lang_code
    conn = None
    try:
        conn = get_pam_db_connection()
        
        # Fetch species that have calculated data in both tables.
        species_query = text("""
            SELECT DISTINCT s.species_id, s.scientific_name, s.common_name_uk, s.common_name_en
            FROM species s
            JOIN species_yearly_trends t ON s.species_id = t.species_id
        ...
        # Fetch available years from trends.
        ...
        # IMPORTANT: render pam_species_dashboard.html.
...
def api_pam_species_dynamics(lang_code):
    ...
    try:
        ...
        # Fetch institution ID parameter if provided.
        ...
        # --- 1. Seasonal activity (dynamic calculation) ---
        # analysis_intermediate is tied to location_id.
        # Filter via JOIN when an institution is selected.
        seasonal_join = ""
        seasonal_where = ""
        ...
        # --- 2. Yearly trend (pre-calculated data) ---
        # Select global trend (IS NULL) or institution-specific (= id).
        ...
        # --- 3. Biotope activity (pre-calculated data) ---
...
def api_data_preview(lang_code):
    ...
    try:
        filters = {
            ...
            'end_date': request.args.get('end_date'),
            'confidence': float(request.args.get('confidence', 0.75)),
            # Multi-select institution filter: '1,2,3' → [1, 2, 3].
            ...
            'aggregation_minutes': request.args.get('aggregation_minutes', 60),  # Default: 60 min.
...
def pam_yearly_table(lang_code):
    """Render the yearly trends table page (page frame only; filters loaded via API)."""
    g.lang_code = lang_code
    try:
        # Years are no longer loaded here; the API handles that.
...
def api_get_trends_filters(lang_code):
    """API endpoint to fetch filter data for the Trends / Overview page."""
    conn = None
    try:
        conn = get_pam_db_connection()
        ...
        # 3. Fetch available institutions (for the dropdown).
        # Honours access rights but ignores selected_institution so switching is possible.
        ...
        # 4. Years (filtered by institution).
        # Append inst_condition to the years query.
        years_query = text(f"""
        ...
        # 7. Taxonomy. species is not directly linked to recordings in this context;
        # taxa are returned globally or filtered via recordings->detections->species (expensive JOIN).
        # Using "available species" logic for now; strict per-institution filtering would require joining detections.
...
def api_yearly_trends_table(lang_code):
    ...
    try:
        ...
        # Handle multi-select institution parameter.
        ...
        # --- Dynamic SQL fragments for institutions ---
        inst_join = ""
        inst_where = ""
        if inst_ids:
        ...
        # --- 2. EFFORT CALCULATION (denominator) ---
        # Count total recording hours for the selected institutions.
        ...
        for row in detections_result:
            ...
            # Normalise: (count / effort_hours) * 24 h.
        ...
        for sid, data in data_by_species.items():
            # Overall index across the full period.
...
def api_get_pam_locations_with_status(lang_code):
    """Return PAM locations with their predicted status; supports ?institution_id=X filtering."""
    g.lang_code = lang_code
    ...
    try:
        ...
        # Additional filter for the institution selected in the dropdown.
        ...
        for loc in locations:
            ...
            if last_visit and last_visit.visit_purpose_id == DEVICE_REMOVED_PURPOSE_ID:
                ...
                # Activity = the most recent of: latest recording OR latest service visit.
                # A fresh install/service counts as activity, so a location is not "dead"
                # just because its last recording is old if the device was recently reinstalled
                # (bug: a reinstall used to show as inactive).
...
def api_get_pam_service_history(lang_code, location_id):
    """API endpoint to fetch service history for a specific PAM location."""
    g.lang_code = lang_code
    conn = None
    try:
        conn = get_pam_db_connection()
...
def api_create_pam_service_visit(lang_code):
    """API endpoint to create a new entry in the PAM service log."""
    g.lang_code = lang_code
    conn = None
    try:
        data = request.json
...
def api_update_pam_service_visit(lang_code, visit_id):
    """API endpoint to update an existing entry in the PAM service log."""
    g.lang_code = lang_code
    conn = None
    try:
        data = request.json
        ...
        # Verify the record exists and the user has access to its location.
...
def api_get_weather_overlay(lang_code):
    """
    API endpoint to fetch weather overlay data.
    Coordinates are determined dynamically:
    1. Centroid of actual detections (if filter results exist).
    2. Centroid of selected locations (if no detections).
    3. Default centre (fallback).
    """
    ...
    try:
        ...
        try:
            ...
            # 3. Attempt 1: find the centroid of actual detections.
            conditions = [
            ...
            # 4. Attempt 2: if no detections, use the centroid of selected locations.
            elif location_ids_str:
                # Re-parse IDs as they may have been consumed above.
                loc_ids = [int(id) for id in location_ids_str.split(',') if id.isdigit()]
                if loc_ids:
        ...
        # 5. Fetch weather data (from DB cache or Open-Meteo API).
        ...
        for row in weather_data:
            ...
            # Wind speed (max and mean).
            result['wind_speed_max'].append(row.get('wind_speed_10m_max'))
            result['wind_speed_mean'].append(row.get('wind_speed_10m_mean'))
...
def export_detailed_data(lang_code):
    """Export dashboard data to a single Excel file with 4 sheets (admin/moderator only)."""
    try:
        # 1. Collect parameters (identical to chart endpoints).
        species_name = request.args.get('species')
        start_date = request.args.get('start_date')
        end_date = request.args.get('end_date')
        institution_id = request.args.get('institution_id', '')
        ...
        # Sheet 2: Daily Dynamics & Weather.
        ...
        # For weather coordinates, use centroid of map_points or the default.
        ...
        # Sheet 4: Raw Detections — reuse get_time_scatter_data since it already contains
        # verification_result and time, covering all points shown in the chart.
        ...
        if not df_time.empty:
            # Map verification status codes to labels.
            ...
            # Join sunrise/sunset data to each detection record by date.
            if not df_sun.empty:
